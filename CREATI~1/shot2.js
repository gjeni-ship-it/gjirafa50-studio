...
const scene="data:image/svg+xml;base64,"+Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1080"><defs><radialGradient id="r" cx="72%" cy="38%" r="72%"><stop offset="0" stop-color="#fdefe0"/><stop offset="55%" stop-color="#efe7df"/><stop offset="100%" stop-color="#e2d6c8"/></radialGradient></defs><rect width="1080" height="1080" fill="url(#r)"/><ellipse cx="780" cy="420" rx="360" ry="300" fill="#ffffff" opacity="0.55"/><ellipse cx="280" cy="880" rx="380" ry="240" fill="#e0d2c0" opacity="0.45"/></svg>').toString('base64');
...
(async()=>{try{
  ...
  await p.evaluate(()=>{document.querySelector('#bgGen').disabled=false;});
  await p.click('#bgGen'); await p.waitForTimeout(800);
